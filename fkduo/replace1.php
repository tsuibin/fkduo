...
switch ($_GET[action]){
case add:
$query=mysql_query("INSERT INTO `{$fkduo}replace` (`oldw`,`type`) VALUES ('$oldw','1')");//更新日志
$bkid=mysql_insert_id();
$eee="replace1.php?action=ok";
header ("location: $eee"); 
break;
...
<table width="44%" height="75" border="1" cellpadding="0" cellspacing="0">
  ...
  <?
$sql="select * FROM `{$fkduo}replace` where `type`='1' ORDER by `id`";
$query=mysql_query($sql);
while ($row=mysql_fetch_array($query)){
?>
    <tr>
      <td><? echo $row[oldw] ?></td>
      <td><a href="replace1.php?action=del&id=<? echo $row[id] ?>">删除</a></td>
    </tr>
  <?
  }
  ?>
...
<form id="form1" name="form1" method="post" action="replace1.php?action=add">
  <label>词语：
